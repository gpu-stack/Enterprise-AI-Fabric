...
class ContextOrchestrator:
    ...
    def generate_chat_response_stream(self, target_adapter: str, chat_history: List[Dict[str, str]], temperature: float, llm_overrides: Dict[str, Any] = None, request_start_time: float = None):
        """Executes token-by-token streaming response for non-RAG chat with TTFT and E2E latency tracking."""
        pre_start = request_start_time or time.perf_counter_ns()
        ...
        try:
            ...
            prompt_tokens = sum(len(m.get("content", "").split()) for m in vllm_payload["messages"])
            completion_tokens = len(response_text.split())
            token_metrics = {
                "prompt_tokens": prompt_tokens,
                "completion_tokens": completion_tokens,
